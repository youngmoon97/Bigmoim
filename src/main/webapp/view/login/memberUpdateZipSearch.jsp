...
<link href="zipSearchStyle.css" rel="stylesheet" type="text/css">
<script type="text/javascript">
	function loadSearch() 
	{
		frm = document.memberUpdate_zipFrm;
		if(frm.area3.value=="")
		{
			alert("도로명을 입력하세요.");
			return;
		}
		frm.action = "memberUpdateZipSearch.jsp";	
		frm.submit();
	}
	
	function sendMemberAdd(zipcode, area1, area2) 
	{
		opener.document.memberUpdateFrm.memberAddrZipcode.value=zipcode; //회원주소 우편번호
		opener.document.memberUpdateFrm.memberAddrArea1.value=area1; //회원주소 area1(시)
		opener.document.memberUpdateFrm.memberAddrArea2.value=area2;//회원주소 area2(구/동)
		self.close();
	}
	
	function sendMemberJobAdd(zipcode, area1, area2) 
	{
		opener.document.memberUpdateFrm.memberJobAddrZipcode.value=zipcode; //직장주소 우편번호
		opener.document.memberUpdateFrm.memberJobAddrArea1.value=area1; //직장주소 area1(시)
		opener.document.memberUpdateFrm.memberJobAddrArea2.value=area2;//직장주소 area2(구/동)
		self.close();
	}
...
<body bgcolor=#FFE7EB>
	<div align="center">
		<br />
		<form name="memberUpdate_zipFrm" method="post">
			<table>
				    <tr>
               <td>
               <br/>동 입력 :    
               ...
               <input type="button" value="검색" onclick="loadSearch()"
				...
				<%}else{%>
				<tr>
					<td align="center" style="padding-bottom: 10px"><br>※검색 후, 아래 주소를 클릭하면 자동으로 입력됩니다.</td>
				</tr>
				<%
				...
				<tr>
					<td align="center"><br/>
					<a href="#" onClick="self.close()" style="color:black">닫기</a></td>
				</tr>
			</table>
